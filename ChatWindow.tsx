...
export default function ChatWindow() {
  ...
  useEffect(() => {
    ...
    const loadChatLogs = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/chat/messages/${currentUserId}/${activeRecipient.user_id}`);
        if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex font-sans w-full overflow-hidden">
      ...
      {/* Dynamic Messaging Inbox Row Left Panel Sidebar */}
      <aside className="w-80 bg-slate-800 border-r border-slate-700 flex flex-col shrink-0">
        <div className="p-4 border-b border-slate-700 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] bg-indigo-500/20 text-indigo-300 font-bold px-2 py-0.5 rounded-full border border-indigo-500/30">{users.length} Chats</span>
          </div>
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-slate-700/60 border border-slate-600/50 rounded-xl pl-9 pr-4 py-1.5 text-xs text-slate-200 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition"
            />
          </div>
        </div>
            ...
            filteredUsers.map((u) => (
              ...
              >
                <div className="relative shrink-0">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-600 font-black text-sm flex items-center justify-center text-white border border-indigo-400/20 uppercase shadow-inner">{u.username.substring(0, 1)}</div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen relative bg-slate-900">
          ...
          {messages.map((msg, i) => {
            ...
            return (
              <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-xs rounded-2xl px-4 py-2.5 text-sm shadow-sm ${isMe ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-slate-800 text-slate-200 rounded-bl-none border border-slate-700'}`}>
                  {msg.isViewOnce ? (
                    ...
                    <div className="space-y-1">
                      {isInferredImage ? (
                        <div className="p-1 bg-slate-900/40 rounded-xl max-w-[240px] cursor-pointer hover:opacity-90 transition" onClick={() => setSelectedImageUrl(msg.content)}>
                          <img src={msg.content} alt="Attachment Preview" className="rounded-lg object-cover max-h-48 w-full border border-slate-700 shadow-sm" />
                        </div>
                      ) : msg.type === 'video' ? (
                        ...
                        <div className="p-1.5 bg-slate-900/40 rounded-xl flex flex-col gap-1 w-48"><div className="flex items-center gap-2 text-emerald-400 text-xs font-bold"><Mic size={14}/> Recorded Audio</div><audio src={msg.content} controls className="w-full h-8 mt-1 scale-95 origin-left" /></div>
                      ) : (
        ...
        <footer className="absolute bottom-0 left-0 right-0 bg-slate-800/90 backdrop-blur-md border-t border-slate-700 p-3 z-40">
          <div className="max-w-2xl mx-auto flex flex-col gap-2">
            <div className="flex items-center gap-1">
              <button type="button" onClick={() => imageInputRef.current?.click()} className="p-2 text-slate-400 hover:text-indigo-400 hover:bg-slate-700/50 rounded-xl transition" title="Upload Image"><Image size={18} /></button>
              <button type="button" onClick={() => videoInputRef.current?.click()} className="p-2 text-slate-400 hover:text-indigo-400 hover:bg-slate-700/50 rounded-xl transition" title="Upload Video"><Film size={18} /></button>
              <button type="button" onClick={isRecording ? stopRecordingAudio : startRecordingAudio} className={`p-2 rounded-xl transition ${isRecording ? 'bg-red-500 text-white animate-pulse' : 'text-slate-400 hover:text-indigo-400 hover:bg-slate-700/50'}`}>{isRecording ? <Square size={18} /> : <Mic size={18} />}</button>
              <button type="button" className="p-2 text-slate-400 hover:text-slate-200 hover:bg-slate-700/50 rounded-xl transition"><Paperclip size={18} /></button>
              <div className="h-4 w-[1px] bg-slate-700 mx-1"></div>
              <button type="button" onClick={() => setViewOnceToggle(!viewOnceToggle)} className={`p-2 rounded-xl transition flex items-center gap-1 text-xs font-bold ${viewOnceToggle ? 'bg-amber-500 text-slate-900 scale-105' : 'bg-slate-700/50 text-amber-400 hover:bg-slate-700'}`}><ShieldAlert size={16} /> View Once</button>
            </div>
            <div className="flex items-center gap-2">
              <button type="button" className="p-2 text-slate-400 hover:text-slate-200 transition"><Smile size={18} /></button>
              <input 
                type="text" 
                value={typedMessage} 
                ...
                className="flex-1 bg-slate-700 rounded-xl px-4 py-2 text-sm text-slate-100 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleSendMessage('text'); } }}
              />
              <button type="button" onClick={() => handleSendMessage('text')} disabled={!typedMessage.trim() || isRecording} className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700 p-2.5 rounded-xl text-white transition"><Send size={16} /></button>
            </div>
          </div>
        </footer>
      </div>
      ...
      {selectedImageUrl && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 transition" onClick={() => setSelectedImageUrl(null)}>
          <button type="button" className="absolute top-4 right-4 text-slate-400 hover:text-white text-sm font-bold bg-slate-800/80 px-3 py-1.5 rounded-xl border border-slate-700 transition">✕ Close Theater View</button>
          <img src={selectedImageUrl} alt="Expanded Media Theater" className="max-w-full max-h-[85vh] rounded-xl object-contain shadow-2xl border border-slate-800" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
    </div>
  );
